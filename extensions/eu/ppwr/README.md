...
├── validation/
│   ├── ppwr-shapes.ttl                       # SHACL shapes
│   ├── ppwr-schema.json                      # hand-written DPP document schema (required/recommended + article clauses)
│   └── ppwr.extension-schema.json            # generated by build:extension-schemas
├── examples/
...
| `euppwr:ReuseInformation` | Article 12(2) reuse data card: re-use system, collection points, trips/rotations. |
| `euppwr:ReuseSystemScope` | Enumeration: LocalSystem / NationalSystem / UnionWideSystem (Article 12(2)). |
...
| `euppwr:averageRotationsEstimate` | `xsd:decimal` | Average estimation where per-item calculation is not feasible (Art. 12(2)); Art. 11(2) delegated act will set format minima. |
