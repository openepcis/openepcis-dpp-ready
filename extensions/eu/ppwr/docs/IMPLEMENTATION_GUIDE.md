...
- **`ppwr:Packaging`** — extends `gs1:Packaging`, a carrier for PPWR data.
- **`ppwr:packagingTier`** — Sales / Grouped / Transport (PPWR Article 3).
- **`ppwr:recyclabilityGrade`** — A / B / C (PPWR Article 4, Annex II).
- **`ppwr:harmonisedSymbol`** — URI of an Annex IX symbol.
...
2. **Packaging tier** — `ppwr:packagingTier` ∈ {Sales, Grouped, Transport}.
3. **Recyclability grade** — `ppwr:recyclabilityGrade` ∈ {A, B, C}
   (mandatory from 2030).
4. **Material composition** — `dpp:MaterialComposition` entries.
5. **Recycled content** — `dpp:RecycledContent` (mandatory for plastic
   packaging from 2030).
6. **EPR registration** — `dpp:extendedProducerResponsibility` for each
...
- **Deposit-return** — `dpp:depositReturnScheme` for beverage containers
  in Member States with DRS.
- **Compostability** — `dpp:Compostability` if the packaging carries an
  industrial / home-compostable claim (note: substantiate with
  `dpp:compostabilityStandard` URI).
- **Bio-based content** — `dpp:bioBasedFraction` (Article 13 optional
  voluntary disclosure).
- **PFAS / SVHC declaration** — `dpp:HazardousSubstance` entries with
...
harmonisedSymbol) and lifted dpp: properties live at **event level**, not
...
- `ppwr:packagingTier` ∈ {Sales, Grouped, Transport}
- `ppwr:recyclabilityGrade` ∈ {A, B, C} (when present)

Lifted `dpp:` shapes (in `extensions/common/core/validation/dpp-core-shapes.ttl`)
cover the rest:

- `dpp:Compostability`, `dpp:Biodegradability`, `dpp:DepositReturnScheme`,
  `dpp:ExtendedProducerResponsibility` — each with their own NodeShape.
- `dpp:QuantitativeValueShape` validates every QuantitativeValue carries
