...
- **Machine-readable registry (RDF)**: [`../vocab/ec-battery-passport-guidance-1.0.ttl`](../vocab/ec-battery-passport-guidance-1.0.ttl) — minted under `https://ref.openepcis.org/vocab/ec-battery-passport-guidance/1.0#` because the Commission publishes no IRIs (same pattern as the GEFEG BatteryPass-Ready mirror). Each data point is dual-typed `rdf:Property` + `cccev:InformationRequirement`.
