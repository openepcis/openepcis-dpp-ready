...
- **Recycled content** — `oec:RecycledContent` (mandatory for some
  product families per delegated acts).
- **Carbon footprint** — `oec:CarbonFootprintDeclaration` with lifecycle
  stages (mandatory for cement and steel from 2027 per delegated acts).
- **End-of-life take-back** — `oec:EndOfLifeProgram`.
- **Substances of concern** — `oec:HazardousSubstance` declarations for
