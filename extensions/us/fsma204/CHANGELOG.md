...
A closed list of codes and the class of things those codes classify sit at different levels, so no graded SKOS relation between them holds in either direction. `usfsma:FoodTraceabilityList` (to `gs1:FruitsVegetables`) now uses `rdfs:seeAlso`.
