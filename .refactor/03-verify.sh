...
for slug in dpp interop battery eudr textile electronics detergent ; do
  count=$(grep -r "https://ref.openepcis.org/extensions/${slug}/" extensions 2>/dev/null | wc -l | tr -d ' ')
  printf "    %-14s %s\n" "${slug}:" "$count"
done
...
grep -rh "^@prefix " extensions --include="*.ttl" | grep "ref.openepcis.org" | sort -u | head -20
