...
The example reads bottom-up: a module property (`eutex:hasSpinningFacility`) ranges to the shared `oec:FacilityInformation`, and the upward anchors live on `oec:`, so every module facility property inherits the GS1, UNTP, and SEMICeu peers for free. The same delegation pattern produces a few other cases worth calling out:
...
| **CSDDD 2024/1760** | ✅ shipped (oec: enrichment) | 0 — enriches `oec:DueDiligenceReport` with `oec:hasDueDiligenceRegulationContext` and `oec:supplyChainTransparencyUrl` |
